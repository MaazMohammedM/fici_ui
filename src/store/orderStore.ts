...
export const useOrderStore = create<OrderState>((set, get) => ({
  ...
  // Admin methods
  adminCancelOrderItem: async (orderItemId: string, reason: string, adminUserId: string) => {
    set({ loading: true, error: null });
    try {
      await updateOrderItemStatus({
        action: 'cancel_item',
  ...
  adminShipOrderItem: async (orderItemId: string, adminUserId: string) => {
    set({ loading: true, error: null });
    try {
      await updateOrderItemStatus({
        action: 'ship_item',
  ...
  adminDeliverOrderItem: async (orderItemId: string, adminUserId: string) => {
    set({ loading: true, error: null });
    try {
      await updateOrderItemStatus({
        action: 'deliver_item',
  ...
  adminRefundOrderItem: async (orderItemId: string, reason: string, adminUserId: string) => {
    set({ loading: true, error: null });
    try {
      await updateOrderItemStatus({
        action: 'refund_item',
