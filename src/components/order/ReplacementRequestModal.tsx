...
interface ReplacementRequestModalProps {
  ...
  onConfirmReplacement: (data: ReplacementRequestData) => void;
...
export const ReplacementRequestModal: React.FC<ReplacementRequestModalProps> = ({
  ...
  const handleConfirm = () => {
    ...
    // Get the reason label from the selected reason code
    const selectedReasonOption = REASON_OPTIONS.find(option => option.value === reasonCode);
    const reasonLabel = selectedReasonOption?.label || '';
    const additionalDetails = replacementReason.trim() || '';
    
    // Combine reason label with additional details
    ...
    onConfirmReplacement(requestData);
  };
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div className="bg-white dark:bg-gray-800 rounded-xl p-4 sm:p-6 max-w-lg w-full mx-4 max-h-[90vh] overflow-y-auto">
