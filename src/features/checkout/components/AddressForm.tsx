...
import { db, collection, getDoc, getDocs, query, where, updateDoc, doc } from '@lib/firebase';
...
const AddressForm: React.FC<Props> = ({ onSelect, selectedId, userProfile, onPhoneUpdateSuccess }) => {
  ...
  const handlePincodeSelect = useCallback((pincodeData: PincodeDetails) => {
    ...
    setValue('pincode', String(pincodeData.pincode));
  ...
  const loadAddresses = async () => {
    ...
    try {
      ...
      if (userDocSnap.exists()) {
        ...
        const arr = Array.isArray(userData?.addresses) ? userData.addresses : [];
  ...
  const upsertAddresses = async (next: Address[]) => {
    ...
    try {
      ...
      await updateDoc(userDocRef, { addresses: next });
